...
class Client_FedAvg_Curr_LG_Pred(object):
    score_data_indices = {};
    train_count = {};
    
    def __init__(self, name, model, local_bs, local_ep, lr0, lr_a, lr_b, momentum, w_decay, device, 
                 lg_scoring, train_ds_local = None, test_dl_local = None, 
        ...
        self.acc_best = 0
        Client_FedAvg_Curr_LG_Pred.train_count[self.name]=0
        self.save_best = True
    ...
    def train(self, w_glob, logger = None):
        Client_FedAvg_Curr_LG_Pred.train_count[self.name]+=1
        ...
        if logger:
            ...
            for idx in Client_FedAvg_Curr_LG_Pred.score_data_indices[self.name]:
                trace = {}
    ...
    def evaluate(self,ind_loss,ds_loader,w_net):
        ...
        for i, (images, labels) in enumerate(ds_loader):
            ...
            list(map(lambda a, b : ind_loss[a].append(int(b.item())), order[start:start+len(labels)], pred!=labels))
            start += len(labels)
