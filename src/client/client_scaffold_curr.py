...
class Client_Scaffold_Curr(object):
    score_data_indices = {};
    train_count = {};
    
    def __init__(self, name, model, local_bs, local_ep, lr0, lr_a, lr_b, momentum, w_decay, device, 
                 lg_scoring, train_ds_local = None, test_dl_local = None, c_local=None,
        ...
        self.acc_best = 0
        Client_Scaffold_Curr.train_count[self.name]=0
        self.save_best = True
    ...
    def train(self, w_glob, c_global, logger = None):
        Client_Scaffold_Curr.train_count[self.name]+=1
        ...
        if logger:
            ...
            for idx in Client_Scaffold_Curr.score_data_indices[self.name]:
                trace = {}
        ...
        while step < myiterations:
            ...
            for batch_idx, (images, labels) in enumerate(ds_loader):
                ...
                for key in net_para:
                    net_para[key] = net_para[key].to(self.device) - lr * (c_global[key] - self.c_local[key])
                self.net.load_state_dict(net_para)
                cnt+=1 
                
                batch_loss.append(loss.item())
                if logger:
                    logger.add_scalars({"loss":loss.item()},f"train_loss/Client{self.name}",step=step);
        ...
        net_para = self.net.state_dict()
        for key in net_para:
            c_new[key] = c_new[key].to(self.device) - c_global[key].to(self.device) + (w_glob[key].to(self.device) - net_para[key].to(self.device))/(cnt* lr)
            c_delta[key] = c_new[key].to(self.device) - self.c_local[key].to(self.device)
            
        self.c_local = copy.deepcopy(c_new)
        self.w_local = copy.deepcopy(self.net.state_dict())
        return sum(epoch_loss) / len(epoch_loss), step, c_delta
    ...
    def scoring_func(self, w_glob=None):
        ...
        # backup
        w_curr_net = copy.deepcopy(self.net.state_dict())
