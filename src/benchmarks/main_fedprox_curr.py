...
def main_fedprox_curr(args,log=None):
    print(' ')
...
#         users_model[idx].load_state_dict(initial_state_dict)
    ...
    if os.path.exists(expert_pt_path):        
        ...
        net_expert = copy.deepcopy(net_glob)
        w_expert,train_loss,test_loss,test_acc,steps= train_model(net_expert,train_ds_global,test_ds_global,
                                                                            lr=0.001,momentum=0.9,num_epoch=10,batch_size=64,num_workers=8,
                                                                            logger=log,device=args.device)
        net_expert.load_state_dict(w_expert);
    ...
    for idx in range(args.num_users):
        ...
        clients.append(Client_FedProx_Curr(idx, copy.deepcopy(users_model[idx]), args.local_bs, args.local_ep, 
    ...
    ################################ Score Client Data
    Client_FedProx_Curr.score_data_indices = {i:np.random.randint(0,
    ...
    duration = end-start
    if log:
        log.add_histogram(torch.tensor(list(Client_FedProx_Curr.train_count.values())),f"Clients/train_count")
    print('-'*40)
    ...
    print('-'*40)
    ############################### FedProx+ (FedProx + FineTuning)
    print('-'*40)
    print('FedProx_Curr+ ::: FedProx_Curr + Local FineTuning')
    sys.stdout.flush()
    ...
    fedprox_ft_client_local = np.mean(temp_acc) 
    fedprox_ft_client_global = np.mean(temp_glob_acc) 
    print(f'-- FedProx_Curr+ :: AVG Client Local Acc: {np.mean(temp_acc):.2f}')
    print(f'-- FedProx_Curr+ :: AVG Client Global Acc: {np.mean(temp_glob_acc):.2f}')
    ...
    return final_glob, avg_final_glob, best_glob, avg_final_local, avg_best_local, fedprox_ft_client_local, fedprox_ft_client_global, duration            

def run_fedprox_curr(args, fname):
    ...
    exp_avg_final_local=[]
    exp_avg_best_local=[]
    exp_fedprox_ft_client_local=[]
    exp_fedprox_ft_client_global=[]
    exp_fl_time=[]
    ...
    for trial in range(args.ntrials):
        ...
        fedprox_ft_client_local, fedprox_ft_client_global, duration = main_fedprox_curr(args,log=log)
        ...
        exp_fedprox_ft_client_local.append(fedprox_ft_client_local)
        exp_fedprox_ft_client_global.append(fedprox_ft_client_global)
        ...
        print(f'-- FedProx_Curr+ Fine Tuning Clients AVG Client Local Acc: {exp_fedprox_ft_client_local[-1]:.2f}')
        print(f'-- FedProx_Curr+ Fine Tuning Clients AVG Client Global Acc: {exp_fedprox_ft_client_global[-1]:.2f}')
    ...
    template = '-- FedProx_Curr+ Fine Tuning Clients AVG Local Acc: {:.2f} +- {:.2f}'
    print(template.format(np.mean(exp_fedprox_ft_client_local), np.std(exp_fedprox_ft_client_local)))
    template = '-- FedProx_Curr+ Fine Tuning Clients AVG Global Acc: {:.2f} +- {:.2f}'
    print(template.format(np.mean(exp_fedprox_ft_client_global), np.std(exp_fedprox_ft_client_global)))
    ...
    with open(fname+'_results_summary.txt', 'a') as text_file:
        ...
        template = '-- FedProx_Curr+ Fine Tuning Clients AVG Local Acc: {:.2f} +- {:.2f}'
        print(template.format(np.mean(exp_fedprox_ft_client_local), np.std(exp_fedprox_ft_client_local)), file=text_file)
        template = '-- FedProx_Curr+ Fine Tuning Clients AVG Global Acc: {:.2f} +- {:.2f}'
        print(template.format(np.mean(exp_fedprox_ft_client_global), np.std(exp_fedprox_ft_client_global)), file=text_file)
