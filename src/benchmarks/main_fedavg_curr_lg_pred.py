...
def main_fedavg_curr_lg_pred(args,log=None):
    print(' ')
...
#         users_model[idx].load_state_dict(initial_state_dict)
    ...
    for idx in range(args.num_users):
        ...
        clients.append(Client_FedAvg_Curr_LG_Pred(idx, copy.deepcopy(users_model[idx]), args.local_bs, args.local_ep, 
    ...
    ################################ Score Client Data
    Client_FedAvg_Curr_LG_Pred.score_data_indices = {i:np.random.randint(0,
    ...
    if log:
        log.add_histogram(torch.tensor(list(Client_FedAvg_Curr_LG_Pred.train_count.values())),f"Clients/train_count")
    print('-'*40)
    ...
    print('-'*40)
    print('FedAvg_Curr_LG_Pred+ ::: FedAvg Curr LG Pred + Local FineTuning')
    sys.stdout.flush()
    ...
    print(f'-- FedAvg_Curr_LG_Pred+ :: AVG Client Local Acc: {np.mean(temp_acc):.2f}')
    print(f'-- FedAvg_Curr_LG_Pred+ :: AVG Client Global Acc: {np.mean(temp_glob_acc):.2f}')
...
def run_fedavg_curr_lg_pred(args, fname):
    ...
    for trial in range(args.ntrials):
        ...
        fedavg_ft_client_local, fedavg_ft_client_global, duration = main_fedavg_curr_lg_pred(args,log=log)
        ...
        print(f'-- FedAvg_Curr_LG_Pred+ Fine Tuning Clients AVG Client Local Acc: {exp_fedavg_ft_client_local[-1]:.2f}')
        print(f'-- FedAvg_Curr_LG_Pred+ Fine Tuning Clients AVG Client Global Acc: {exp_fedavg_ft_client_global[-1]:.2f}')
    ...
    template = '-- FedAvg_Curr_LG_Pred+ Fine Tuning Clients AVG Local Acc: {:.2f} +- {:.2f}'
    print(template.format(np.mean(exp_fedavg_ft_client_local), np.std(exp_fedavg_ft_client_local)))
    template = '-- FedAvg_Curr_LG_Pred+ Fine Tuning Clients AVG Global Acc: {:.2f} +- {:.2f}'
