...
#         users_model[idx].load_state_dict(initial_state_dict)
    ...
    client_curriculum = ClientScheduler(client_set,total_budget,ordering=args.client_ordering,
    ...
    while iteration < total_budget:        
        ...
        current_client_set = client_curriculum.next_sched(w_glob=w_glob,curr_step=iteration);

        client_loader = DataLoader(current_client_set,batch_size=args.client_bs,shuffle=True);
    ...
    print('-'*40)
    print('FedAvg_Curr+ ::: FedAvg Curr + Local FineTuning')
    sys.stdout.flush()
    ...
    print(f'-- FedAvg_Curr+ :: AVG Local Acc: {np.mean(local_acc):.2f}')
...
def run_fedavg_curr(args, fname):
    ...
    for trial in range(args.ntrials):
        ...
        fedavg_ft_local, duration = main_fedavg_curr(args)
        ...
        print(f'-- FedAvg_Curr+ Fine Tuning Clients AVG Local Acc: {exp_fedavg_ft_local[-1]:.2f}')
    ...
    template = '-- FedAvg_Curr+ Fine Tuning Clients AVG Local Acc: {:.2f} +- {:.2f}'
