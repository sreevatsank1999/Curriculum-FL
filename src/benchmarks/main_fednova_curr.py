...
def main_fednova_curr(args,log=None):
    print(' ')
...
#         users_model[idx].load_state_dict(initial_state_dict)
    ...
    for idx in range(args.num_users):
        ...
        clients.append(Client_FedNova_Curr(idx, copy.deepcopy(users_model[idx]), args.local_bs, args.local_ep, 
    ...
    ################################ Score Client Data
    Client_FedNova_Curr.score_data_indices = {i:np.random.randint(0,
    ...
    while nbupdate < total_budget:
        ...
        for n,client_batch in enumerate(client_loader):                    
            ...
            for client in client_batch:
                loss,steps,a_i,d_i = client.train(w_glob=copy.deepcopy(w_glob),logger=log)
                loss_locals.append(copy.deepcopy(loss))                                      
                steps_local.append(copy.deepcopy(steps))                                      

                a_list.append(a_i)
                d_list.append(d_i)
                n_i = len(net_dataidx_map[idx])
                n_list.append(n_i)
            ...
            for key in updated_model:
                ...
                updated_model[key] -= (coeff * d_total_round[key]).to(args.device)
    ...
    duration = end-start
    if log:
        log.add_histogram(torch.tensor(list(Client_FedNova_Curr.train_count.values())),f"Clients/train_count")
    print('-'*40)
    ...
    steps_local = [];
    for idx in range(args.num_users): 
        _,steps,_,_ = clients[idx].train(w_glob=copy.deepcopy(w_glob),logger=log)
        steps_local.append(copy.deepcopy(steps))   
    ...
    fednova_ft_client_local = np.mean(temp_acc) 
    fednova_ft_client_global = np.mean(temp_glob_acc) 
    print(f'-- FedNova_Curr+ :: AVG Client Local Acc: {np.mean(temp_acc):.2f}')
    print(f'-- FedNova_Curr+ :: AVG Client Global Acc: {np.mean(temp_glob_acc):.2f}')
    ...
    return final_glob, avg_final_glob, best_glob, avg_final_local, avg_best_local, fednova_ft_client_local, fednova_ft_client_global, duration

def run_fednova_curr(args, fname):
    ...
    exp_avg_final_local=[]
    exp_avg_best_local=[]
    exp_fednova_ft_client_local=[]
    exp_fednova_ft_client_global=[]
    exp_fl_time=[]
    ...
    for trial in range(args.ntrials):
        ...
        fednova_ft_client_local, fednova_ft_client_global, duration = main_fednova_curr(args,log=log)
        ...
        exp_fednova_ft_client_local.append(fednova_ft_client_local)
        exp_fednova_ft_client_global.append(fednova_ft_client_global)
        ...
        print(f'-- FedNova_Curr+ Fine Tuning Clients AVG Client Local Acc: {exp_fednova_ft_client_local[-1]:.2f}')
        print(f'-- FedNova_Curr+ Fine Tuning Clients AVG Client Global Acc: {exp_fednova_ft_client_global[-1]:.2f}')
    ...
    template = '-- FedNova_Curr+ Fine Tuning Clients AVG Local Acc: {:.2f} +- {:.2f}'
    print(template.format(np.mean(exp_fednova_ft_client_local), np.std(exp_fednova_ft_client_local)))
    template = '-- FedNova_Curr+ Fine Tuning Clients AVG Global Acc: {:.2f} +- {:.2f}'
    print(template.format(np.mean(exp_fednova_ft_client_global), np.std(exp_fednova_ft_client_global)))
    ...
    with open(fname+'_results_summary.txt', 'a') as text_file:
        ...
        template = '-- FedNova_Curr+ Fine Tuning Clients AVG Local Acc: {:.2f} +- {:.2f}'
        print(template.format(np.mean(exp_fednova_ft_client_local), np.std(exp_fednova_ft_client_local)), file=text_file)
        template = '-- FedNova_Curr+ Fine Tuning Clients AVG Global Acc: {:.2f} +- {:.2f}'
        print(template.format(np.mean(exp_fednova_ft_client_global), np.std(exp_fednova_ft_client_global)), file=text_file)
