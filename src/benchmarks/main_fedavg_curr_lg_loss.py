...
def main_fedavg_curr_lg_loss(args,log=None):
    print(' ')
...
#         users_model[idx].load_state_dict(initial_state_dict)
    ...
    if os.path.exists(expert_pt_path):        
        ...
        net_expert = copy.deepcopy(net_glob)
        w_expert,train_loss,train_acc,test_loss,test_acc,steps= train_model(net_expert,train_ds_global,test_ds_global,'expert',
    ...
    if log:
        log.add_histogram(torch.tensor(list(Client_FedAvg_Curr_LG_Loss.train_count.values())),f"Clients/train_count")
    print('-'*40)
    ...
    print('-'*40)
    print('FedAvg_Curr_LG_Loss+ ::: FedAvg Curr LG Loss + Local FineTuning')
    sys.stdout.flush()
    ...
    print(f'-- FedAvg_Curr_LG_Loss+ :: AVG Client Local Acc: {np.mean(temp_acc):.2f}')
    print(f'-- FedAvg_Curr_LG_Loss+ :: AVG Client Global Acc: {np.mean(temp_glob_acc):.2f}')
...
def run_fedavg_curr_lg_loss(args, fname):
    ...
    for trial in range(args.ntrials):
        ...
        fedavg_ft_client_local, fedavg_ft_client_global, duration = main_fedavg_curr_lg_loss(args,log=log)
        ...
        print(f'-- FedAvg_Curr_LG_Loss+ Fine Tuning Clients AVG Client Local Acc: {exp_fedavg_ft_client_local[-1]:.2f}')
        print(f'-- FedAvg_Curr_LG_Loss+ Fine Tuning Clients AVG Client Global Acc: {exp_fedavg_ft_client_global[-1]:.2f}')
    ...
    template = '-- FedAvg_Curr_LG_Loss+ Fine Tuning Clients AVG Local Acc: {:.2f} +- {:.2f}'
    print(template.format(np.mean(exp_fedavg_ft_client_local), np.std(exp_fedavg_ft_client_local)))
    template = '-- FedAvg_Curr_LG_Loss+ Fine Tuning Clients AVG Global Acc: {:.2f} +- {:.2f}'
