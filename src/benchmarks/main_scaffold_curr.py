...
def main_scaffold_curr(args,log=None):
    print(' ')
...
#         users_model[idx].load_state_dict(initial_state_dict)
    ...
    for key in initial_state_dict.keys():
        c_global[key] = torch.zeros_like(initial_state_dict[key],dtype=torch.float).to(args.device)
    ...
    for idx in range(args.num_users):
        ...
        clients.append(Client_Scaffold_Curr(idx, copy.deepcopy(users_model[idx]), args.local_bs, args.local_ep, 
                                           args.lr0,args.lr_sched_a,args.lr_sched_b,args.momentum,args.w_decay,
                                           args.device, args.lg_scoring, train_ds_local, test_dl_local,
                                           c_local,
    ...
    ################################ Score Client Data
    Client_Scaffold_Curr.score_data_indices = {i:np.random.randint(0,
    ...
    while nbupdate < total_budget:
        ...
        for n,client_batch in enumerate(client_loader):                    
            ...
            for client in client_batch:
                loss,steps,c_delta = client.train(w_glob=copy.deepcopy(w_glob),c_global=copy.deepcopy(c_global),logger=log)
            ...
            for key in total_delta:
                total_delta[key] /= len(client_batch)
            ...
            w_diff = []
            for client in client_batch:
                wc = copy.deepcopy(client.get_state_dict())
                wc_vec=[]
            ...
            ####### FedAvg ####### START
            total_data_points = sum([len(net_dataidx_map[client.name]) for client in client_batch])
            fed_avg_freqs = [len(net_dataidx_map[client.name]) / total_data_points for client in client_batch]
            w_locals = []
            for client in client_batch:
                w_locals.append(copy.deepcopy(client.get_state_dict()))

            ww = FedAvg(w_locals, weight_avg=fed_avg_freqs)
            w_glob = copy.deepcopy(interpol_weights(w_glob,ww,alpha=args.glob_momentum))
    ...
    if log:
        log.add_histogram(torch.tensor(list(Client_Scaffold_Curr.train_count.values())),f"Clients/train_count")
    print('-'*40)
    ...
    steps_local = [];
    for idx in range(args.num_users): 
        _,steps,c_delta = clients[idx].train(w_glob=copy.deepcopy(w_glob),c_global=copy.deepcopy(c_global),logger=log)
        steps_local.append(copy.deepcopy(steps))   
    ...
    scaffold_ft_client_local = np.mean(temp_acc) 
    scaffold_ft_client_global = np.mean(temp_glob_acc) 
    print(f'-- Scaffold+ :: AVG Client Local Acc: {np.mean(temp_acc):.2f}')
    print(f'-- Scaffold+ :: AVG Client Global Acc: {np.mean(temp_glob_acc):.2f}')
    ...
    return final_glob, avg_final_glob, best_glob, avg_final_local, avg_best_local, scaffold_ft_client_local, scaffold_ft_client_global, duration

def run_scaffold_curr(args, fname):
    ...
    exp_avg_final_local=[]
    exp_avg_best_local=[]
    exp_scaffold_ft_client_local=[]
    exp_scaffold_ft_client_global=[]
    exp_fl_time=[]
    ...
    for trial in range(args.ntrials):
        ...
        scaffold_ft_client_local, scaffold_ft_client_global, duration = main_scaffold_curr(args,log=log)
        ...
        exp_scaffold_ft_client_local.append(scaffold_ft_client_local)
        exp_scaffold_ft_client_global.append(scaffold_ft_client_global)
        ...
        print(f'-- Scaffold+ Fine Tuning Clients AVG Client Local Acc: {exp_scaffold_ft_client_local[-1]:.2f}')
        print(f'-- Scaffold+ Fine Tuning Clients AVG Client Global Acc: {exp_scaffold_ft_client_global[-1]:.2f}')
    ...
    print(template.format(np.mean(exp_scaffold_ft_client_local), np.std(exp_scaffold_ft_client_local)))
    template = '-- Scaffold+ Fine Tuning Clients AVG Global Acc: {:.2f} +- {:.2f}'
    print(template.format(np.mean(exp_scaffold_ft_client_global), np.std(exp_scaffold_ft_client_global)))
    ...
    with open(fname+'_results_summary.txt', 'a') as text_file:
        ...
        template = '-- Scaffold+ Fine Tuning Clients AVG Local Acc: {:.2f} +- {:.2f}'
        print(template.format(np.mean(exp_scaffold_ft_client_local), np.std(exp_scaffold_ft_client_local)), file=text_file)
        template = '-- Scaffold+ Fine Tuning Clients AVG Global Acc: {:.2f} +- {:.2f}'
        print(template.format(np.mean(exp_scaffold_ft_client_global), np.std(exp_scaffold_ft_client_global)), file=text_file)
