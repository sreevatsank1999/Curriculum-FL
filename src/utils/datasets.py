...
import numpy as np
from torchvision.datasets import MNIST, CIFAR10, SVHN, FashionMNIST, CIFAR100, ImageFolder, DatasetFolder, USPS, STL10
...
from src.transform import AddGaussianNoise
...
class MNIST_rotated(data.Dataset):
    ...
    def __init__(self, root, rotation=0, dataidxs=None, train=True, transform=None, target_transform=None, download=False):
...
class CIFAR100_levelnoise(data.Dataset):

    def __init__(self, root, dataidxs=None, train=True, transform=None, target_transform=None, download=False,rngseed=1,nlevels=[0.0,0.05,0.1,0.2,0.4]):
    ...
    def __build_truncated_dataset__(self):
        ...
        rng = np.random.RandomState(self.rngseed);
        data_nlevel = rng.randint(0,len(self.nlevels),size=len(cifar_dataobj.data));
        
        datan = np.zeros(data.shape,dtype=np.float32);
        for _i in range(len(self.nlevels)):
            indx = np.where(data_nlevel == _i)[0];
            noise_transform = transforms.Compose([
                transforms.ToTensor(),
                transforms.Normalize(mean=[0.507, 0.487, 0.441], std=[0.267, 0.256, 0.276]),
                AddGaussianNoise(0., self.nlevels[_i], None, 0)
            ])
            for _j in indx:
                if self.dataidxs is not None:
                    if _j in self.dataidxs:
                        datan[_j]=noise_transform(data[_j]).numpy().transpose();
                else:
                    datan[_j]=noise_transform(data[_j]).numpy().transpose();
                    

        if self.dataidxs is not None:
            datan = datan[self.dataidxs]
            target = target[self.dataidxs]
...
class STL10_truncated(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        ...
        if self.train:
            ...
            split='test'
        stl10_dataobj = STL10(self.root, split=split, transform=self.transform, 
