...
class Client_Scaffold_Curr(object):
    def __init__(self, name, model, local_bs, local_ep, lr, momentum, device,
                 ...
                 ordering='curr', pacing_f='linear', pacing_a=0.2, pacing_b=0.4, c_local=None):
        self.name = name 
        self.net = model
    ...
    def train(self, w_glob, c_global, is_print = False):
        ...
        if self.w_local == None:
            w_glob = self.net.state_dict()
            
            order = self.scoring_func(w_glob=w_glob)
            ...
            order = self.scoring_func(w_glob=w_glob, w_local=self.w_local)
            if self.ordering == "curr":
                order = order
            elif self.ordering == "random":
                np.random.shuffle(order)
        ...
        self.w_local = self.net.state_dict()
    ...
    def scoring_func(self, w_glob=None, w_local=None):
        ...
        with torch.no_grad():
            ...
            if w_local != None:
                ...
                for i, (images, labels) in enumerate(ds_loader):
                    ...
                    list(map(lambda a, b : ind_loss[a].append(b.item()), order[start:start+len(labels)], indloss))
                    start += len(labels)
        stat = {k:sum(v) for k, v in sorted(ind_loss.items(), key=lambda item:sum(item[1]))}
